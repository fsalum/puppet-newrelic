...
#   Note that, if you specifiy a specific version, you will need to keep this regularly updated, as NewRelic only retain the last two
#   releases on their download site.
#
...
class newrelic::agent::dotnet (
  ...
  case $newrelic_dotnet_package_ensure {
    ...
    default:    {
      $package_source   = "${newrelic_dotnet_source}/NewRelicDotNetAgent_${facts['os']['architecture']}
        _${newrelic_dotnet_package_ensure}.msi"
